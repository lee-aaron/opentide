...
func ConnectPool(ctx context.Context, dsn string, logger *slog.Logger) (*pgxpool.Pool, error) {
	...
	// Log connection info for debugging managed database issues.
	var currentUser, currentDB string
	_ = pool.QueryRow(ctx, `SELECT current_user, current_database()`).Scan(&currentUser, &currentDB)
	logger.Info("postgres connected", "user", currentUser, "database", currentDB)
